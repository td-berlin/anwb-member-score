...
def main():
    ...
    if not upload_file(f'{dir_path}/bootstrap_emr/bootstrap_actions.sh', bucket_name, 'bootstrap_actions.sh'):
    ...
    if not create_stack(stack_name, cfn_template_path, cfn_params_path, ec2_key_name, bucket_name, project_name, environment):
        ...
        response = bucket_tagging.put(
            Tagging={
                'TagSet': [
                    {
                        'Key': 'Name',
                        'Value': f'{project_name} Bootstrap Bucket'
                    },
                ]
            }
        )
...
def create_stack(stack_name, cfn_template, cfn_params_path, ec2_key_name, bucket_name, project_name, environment):
    ...
    cfn_params.append({'ParameterKey': 'EnvironmentName', 'ParameterValue': environment.capitalize()})
