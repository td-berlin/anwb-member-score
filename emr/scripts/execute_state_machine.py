...
def main():
    args = parse_args()
    state_machine_arn = get_state_machine_arn(args.state_machine)
...
def start_execution(state_machine_arn, input_params):
    ...
    try:
        response = step_functions_client.start_execution(
            stateMachineArn=state_machine_arn, name=f"e-{str(uuid.uuid4())}", input=input_params
        )
...
def get_state_machine_arn(state_machine_name):
    ...
    state_machine_arn = (
        list(filter(lambda arn: arn["name"] == state_machine_name, state_machines["stateMachines"]))
    )[0]["stateMachineArn"]
...
def parse_args():
    ...
    parser.add_argument(
        "-s", "--state-machine", required=True, help="Name of state machine to execute"
    )
    parser.add_argument(
        "-i", "--inputs-file", required=True, help="Name of state machine inputs file"
    )
